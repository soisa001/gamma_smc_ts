...
# Space-constrained runner for one run4 arm.
#
...
#
# Usage: bash scripts/run_run4_arm.sh <arm_id> [workers]

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ARM="${1:?usage: run_run4_arm.sh <arm_id> [workers]}"
WORKERS="${2:-8}"
...
run_phase() {
    "$UV" run --project "$REPO" --frozen --all-extras --no-sync \
        python "$REPO/scripts/run_run4.py" "$@"
}
...
find "$REPO/sim_results_run4/$ARM" -name simulation.trees -delete
df -h "$REPO" | tail -1

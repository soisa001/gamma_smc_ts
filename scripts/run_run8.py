#!/usr/bin/env python
"""Simulate run8 and decode each replicate with Gamma-SMC on the same pairs.
...
a 10 Mb contig would otherwise be several gigabytes of dead intermediate.
    ...
    python scripts/run_run8.py --slim-bin .native-stdpopsim/bin/slim \\
        --decoder bin/gamma_smc --workers 6
"""
...
from gamma_smc_aou.run8_models import (  # noqa: E402
    build_contig,
    build_events,
...
STUDY_ROOT = REPO / "sim_results_run8"
...
def run_one(task: Task) -> dict:
    ...
    try:
        ...
        endpoint = {
            "schema": "gamma-smc.run8-endpoint/v2",

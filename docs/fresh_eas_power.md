...
All logs and temporary files are on D. `run_status.json` records progress; `sample_manifest.json` records planned replicate identities and seeds. A storage check stops new submissions before reaching the 3 TB aggregate limit, retaining 20 GB of free/reserved space. It conservatively counts all used space on the output volume, avoiding repeated scans of every simulation artifact over the Windows/WSL filesystem bridge. Other files on that volume therefore also consume this conservative allowance. Restarting WSL still stops a running process; rerun the same command to resume.

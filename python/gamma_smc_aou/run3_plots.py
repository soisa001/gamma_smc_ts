...
at all -- which in run3 is the difference between including and excluding the
...
from .run3_analysis import RESULT_OUTPUTS, cross_arm_power
from .run3_config import ARMS, SIGNIFICANCE_LEVEL, Run3Arm, TMRCA_THRESHOLDS_YEARS
...
def plot_final_af(arm: Run3Arm, results: Path, figures: Path) -> dict[str, Path]:
...
def plot_power_by_estimator(arm: Run3Arm, results: Path, figures: Path) -> dict[str, Path]:
...
def plot_truth_vs_decoded(arm: Run3Arm, results: Path, figures: Path) -> dict[str, Path]:
...
def plot_spatial_profile(
    arm: Run3Arm, results: Path, figures: Path,
...
def plot_arm(study_root: str | Path, arm: Run3Arm) -> dict[str, Any]:
...
def plot_cross_arm(study_root: str | Path) -> dict[str, Path]:
    ...
    fig.suptitle("run3 — tree-truth power by arm", y=1.02)

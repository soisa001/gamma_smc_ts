...
at all -- which in run4 is the difference between including and excluding the
...
from .run4_analysis import RESULT_OUTPUTS, cross_arm_power
from .run4_config import ARMS, SIGNIFICANCE_LEVEL, Run4Arm, TMRCA_THRESHOLDS_YEARS
...
def plot_final_af(arm: Run4Arm, results: Path, figures: Path) -> dict[str, Path]:
...
def plot_power_by_estimator(arm: Run4Arm, results: Path, figures: Path) -> dict[str, Path]:
...
def plot_truth_vs_decoded(arm: Run4Arm, results: Path, figures: Path) -> dict[str, Path]:
...
def plot_spatial_profile(
    arm: Run4Arm, results: Path, figures: Path,
...
def plot_arm(study_root: str | Path, arm: Run4Arm) -> dict[str, Any]:
...
def plot_cross_arm(study_root: str | Path) -> dict[str, Path]:
    ...
    fig.suptitle("run4 — tree-truth power by arm", y=1.02)

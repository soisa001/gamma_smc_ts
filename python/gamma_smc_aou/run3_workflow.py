"""Phase orchestration for the run3 study.
...
from .run3_decode import build_decode_tasks, run_decode_tasks
from .run3_models import (
    generate_slim_script,
    model_record,
    tick_schedule,
)
from .run3_simulate import build_tasks, check_environment, run_tasks

DEFAULT_STUDY_DIRNAME = "sim_results_run3"
...
def check_slim_script(arm: Run3Arm, mode: str, script: str) -> dict[str, Any]:
    """Assert the run3 invariants against a generated SLiM script."""
    ...
    if "c()" not in conditioning:
        raise AssertionError("run3 must not condition on allele frequency")
    ...
    # The allele exists in BOTH modes; only the fitness callback differs.
    if drawn_rows != 1:
    ...
    if "run3_placement_frequency" not in script:
        raise AssertionError("the placement patch is not in the script")
    if "run3_final_census_af" not in script:
...
def phase_validate(
    ...
    *,
    ...
    report: dict[str, Any] = {
        "schema": "gamma-smc.run3-validation/v1",
...
def phase_analyze(repo_root: str | Path, *, arms: Iterable[Run3Arm] = ARMS):
    root = study_root(repo_root)
    written: dict[str, Any] = {}
    for arm in arms:
        written[arm.arm_id] = {
            k: str(v) for k, v in run3_analysis.analyse_arm(root, arm).items()
        }
    cross = run3_analysis.cross_arm_power(root, ARMS)
...
def phase_plot(repo_root: str | Path, *, arms: Iterable[Run3Arm] = ARMS):
    from . import run3_plots

    root = study_root(repo_root)
    written = {arm.arm_id: run3_plots.plot_arm(root, arm) for arm in arms}
    try:
        written["cross_arm"] = run3_plots.plot_cross_arm(root)

"""Replicate execution for the run5 study.
...
from .run4_models import reserved_positions, scoped_reserved_overlay_patch
...
def focal_allele_counts(
    ...
    In run5 the allele exists in both modes, but archaic ancestry is frequently
...
def run_replicate(task: ReplicateTask) -> dict[str, Any]:
    ...
    try:
        ...
        if arm.engine == "msprime":
            # The EAS reference arm needs no forward simulation: it masks the
            # focal base in its own rate map, so there is no overlay to guard.
            ts = simulate_msprime(arm, task.mode, task.seed, task.repo_root)
            slim_patch = {"engine": "msprime", "patched": False}
            overlay = {
                "engine": "msprime",
                "intercepted_calls": 0,
                "masked_calls": 0,
                "note": "focal base excluded by the arm's own mutation rate map",
            }
        else:
            ...
            with (
                ...
                scoped_reserved_overlay_patch((FOCAL_POSITION_BP,)) as overlay,
            ):
                ts = engine.simulate(
                    model,
                    contig,
        ...
        endpoint: dict[str, Any] = {
            "schema": "gamma-smc.run5-endpoint/v1",
            ...
            "placement": {
                "tick": _scalar(block, "run5_placement_tick"),
                "population_id": _scalar(block, "run5_placement_population_id"),
                "mode": _scalar(block, "run5_placement_mode"),
                "carrier_genomes": _scalar(block, "run5_placement_carrier_genomes"),
                "population_genomes": _scalar(block, "run5_placement_population_genomes"),
                "frequency": _scalar(block, "run5_placement_frequency"),
            },
            "final_allele_frequency": {
                "census_alt_count": _scalar(block, "run5_final_census_alt_count"),
                "census_total_count": _scalar(block, "run5_final_census_total_count"),
                "census_af": _scalar(block, "run5_final_census_af"),
                "census_state": _scalar(block, "run5_final_census_state"),
...
def check_environment(slim_path: str | Path, decoder_path: str | Path | None = None):
    ...
    from .run5_config import REQUIRED_SLIM_VERSION, REQUIRED_STDPOPSIM_VERSION
    ...
    if REQUIRED_SLIM_VERSION not in reported:
        raise RuntimeError(
            f"run5 requires SLiM {REQUIRED_SLIM_VERSION}; {slim} reported: {reported}"
        )
    record = {
        "slim_path": str(slim),

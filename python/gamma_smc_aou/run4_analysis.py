"""Statistics, null distribution and p-values for the run4 study.

Everything here reads the *raw* per-pair TMRCA arrays written by
``run4_simulate`` and the raw decoder summaries written by ``run4_decode``, so
...
from .run4_simulate import GENOTYPE_CLASSES, REPLICATE_OUTPUTS
...
def _replicate_directories(study_root: str | Path, arm: Run4Arm, mode: str) -> list[Path]:
...
def replicate_status(study_root: str | Path, arm: Run4Arm) -> pd.DataFrame:
    rows = []
    for mode in MODES:
        for directory in _replicate_directories(study_root, arm, mode):
            path = directory / REPLICATE_OUTPUTS["endpoint"]
...
def final_allele_frequencies(study_root: str | Path, arm: Run4Arm) -> pd.DataFrame:
    rows = []
    for mode in MODES:
        for directory in _replicate_directories(study_root, arm, mode):
            endpoint = _load_endpoint(directory)
...
def null_distribution(statistics: pd.DataFrame, arm: Run4Arm) -> pd.DataFrame:
    rows = []
...
def pvalues(
    statistics: pd.DataFrame, arm: Run4Arm, *, condition_on_presence: bool = False
) -> pd.DataFrame:
    ...
    focal allele at all.  In run4 that is a substantive restriction: archaic
...
def power_summary(pvalue_table: pd.DataFrame, arm: Run4Arm) -> pd.DataFrame:
...
def null_calibration(statistics: pd.DataFrame, arm: Run4Arm) -> pd.DataFrame:
...
def truth_vs_decoded(statistics: pd.DataFrame, arm: Run4Arm) -> pd.DataFrame:
...
def analyse_arm(study_root: str | Path, arm: Run4Arm) -> dict[str, Path]:
...
def cross_arm_power(study_root: str | Path, arms: Iterable[Run4Arm]) -> pd.DataFrame:
    frames = []
    for arm in arms:

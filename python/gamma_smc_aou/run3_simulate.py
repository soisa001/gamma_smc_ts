"""Replicate execution for the run3 study.
...
def focal_allele_counts(
    ...
    In run3 the allele exists in both modes, but archaic ancestry is frequently
...
def run_replicate(task: ReplicateTask) -> dict[str, Any]:
    ...
    try:
        ...
        endpoint: dict[str, Any] = {
            "schema": "gamma-smc.run3-endpoint/v1",
            ...
            "placement": {
                "tick": _scalar(block, "run3_placement_tick"),
                "population_id": _scalar(block, "run3_placement_population_id"),
                "mode": _scalar(block, "run3_placement_mode"),
                "carrier_genomes": _scalar(block, "run3_placement_carrier_genomes"),
                "population_genomes": _scalar(block, "run3_placement_population_genomes"),
                "frequency": _scalar(block, "run3_placement_frequency"),
            },
            "final_allele_frequency": {
                "census_alt_count": _scalar(block, "run3_final_census_alt_count"),
                "census_total_count": _scalar(block, "run3_final_census_total_count"),
                "census_af": _scalar(block, "run3_final_census_af"),
                "census_state": _scalar(block, "run3_final_census_state"),
...
def check_environment(slim_path: str | Path, decoder_path: str | Path | None = None):
    ...
    from .run3_config import REQUIRED_SLIM_VERSION, REQUIRED_STDPOPSIM_VERSION
    ...
    if REQUIRED_SLIM_VERSION not in reported:
        raise RuntimeError(
            f"run3 requires SLiM {REQUIRED_SLIM_VERSION}; {slim} reported: {reported}"
        )
    record = {
        "slim_path": str(slim),

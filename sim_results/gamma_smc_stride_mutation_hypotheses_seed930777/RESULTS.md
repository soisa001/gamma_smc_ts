...
- At `mu=1.25e-8`, the same selected genealogy is recovered at 1 kb:
  selected `0.071757`, null mean `0.021938`, 0/100 null exceedances, and
  `p=1/101=0.009901`.
- An independent high-mutation 10 kb selected decode is `0.071845`, only 0.12%
  above the high-mutation 1 kb center. It therefore retains the selected
  signal. The formal 100-null high-mutation calibration was run at 1 kb.
...
| high mu, 10 kb | `0.07184464` | not rerun | not rerun | not rerun | selected-only |
...
- Across the 100,000 shared low-mutation null rows, center values correlate
  0.972 between strides.
...
cell reaches it. In the low-mutation 10 kb cell, 89/100 nulls exceed the
selected center. The exact 95% interval for the underlying exceedance
probability is 0.812-0.944, so adding more nulls would refine a large p-value;
it would not make this center significant.
...
- The optimized 100-null high-mutation 1 kb study completed in 341.0 s with
  12 workers. This is an observed workflow time, not a controlled before/after
  benchmark.
...
- `low_mu_stride1kb/` and `high_mu_stride1kb/`: complete selected and 100-null
  profiles, pointwise scans, density tests, plots, and metrics.
- `high_mu_stride10kb_selected_only/`: independent 10 kb selected profile.
